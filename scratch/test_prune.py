...
def _prune_spurs(G: nx.Graph, theta: float = 1.2) -> nx.Graph:
    G_pruned = G.copy()
    while True:
...
def get_skeleton_pruned(polygon, spacing=1.0, theta=1.2, min_branch_length=4.0):
...
def plot_text(text, filename):
    polys = text_to_outline_polygons(text, "assets/fonts/UTM ThuPhap Thien An.ttf", 200)
    geom = MultiPolygon(polys)
    
    # 2. Get skeleton
    strokes = get_skeleton_pruned(geom, spacing=1.0, theta=1.5, min_branch_length=4.0)
    print(f"Text '{text}' generated {len(strokes)} strokes.")
    ...
    ax.set_aspect("equal")
    ax.set_title(f"Pruned Voronoi Skeleton: '{text}'")
    ax.legend()
    plt.tight_layout()
...
if __name__ == "__main__":
    plot_text("nhẫn", "output/nhan_pruned.png")
    plot_text("Tâm", "output/tam_pruned.png")
