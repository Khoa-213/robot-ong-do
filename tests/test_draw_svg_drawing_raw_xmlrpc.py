...
DEFAULT_SVG_PATH = PROJECT_ROOT / "assets" / "svg" / "drawing.svg"


def main() -> None:
    parser = argparse.ArgumentParser(description="Draw an SVG file as separate writing strokes.")
    ...
    print("[RAW_SVG_DRAWING] Config:", CONFIG_PATH)
    print("[RAW_SVG_DRAWING] SVG:", svg_path)
    print("[RAW_SVG_DRAWING] Raw stroke count:", len(strokes))
    print("[RAW_SVG_DRAWING] Raw pose count:", len(poses))
    print("[RAW_SVG_DRAWING] Planned stroke count:", len(planned_strokes))
    print("[RAW_SVG_DRAWING] Planned pose count:", planned_pose_count)
    print("[RAW_SVG_DRAWING] Measured paper width/height:", round(width, 3), round(height, 3))
    print("[RAW_SVG_DRAWING] First pose:", poses[0])
    print("[RAW_SVG_DRAWING] Last pose:", poses[-1])
    print("[RAW_SVG_DRAWING] Start pose:", start_pose)
    print("[RAW_SVG_DRAWING] Return pose:", return_pose)
    print("[RAW_SVG_DRAWING] enable_robot_move:", enable_move)
    print("[RAW_SVG_DRAWING] allow_raw_xmlrpc_motion:", allow_raw_motion)
    print("[RAW_SVG_DRAWING] blend_radius:", blend_radius)
    print("[RAW_SVG_DRAWING] dry_run:", args.dry_run)
    ...
    print("[RAW_SVG_DRAWING] Safety validation OK")

    if not enable_move or not allow_raw_motion:
        print("[RAW_SVG_DRAWING] SAFETY LOCK: raw XML-RPC movement disabled")
        print("[RAW_SVG_DRAWING] This script will not send robot motion")
        if args.dry_run:
            print("[RAW_SVG_DRAWING] DRY RUN complete")
            return
    elif not args.yes:
        confirmation = input("Type RUN to start real robot motion: ").strip()
        if confirmation != "RUN":
            print("[RAW_SVG_DRAWING] Motion cancelled by user")
            return
